...
constexpr auto dot(std::array<float, 3> const &a, std::array<float, 3> const &b)
    -> float {
  return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
}

[[nodiscard]] auto angular_error_degrees(std::array<float, 3> const &a,
                                         std::array<float, 3> const &b)
    -> float {
  auto const dot_product = std::clamp(dot(a, b), -1.0f, 1.0f);

  constexpr auto radians_to_degrees = 57.2957795f;

  return std::acos(dot_product) * radians_to_degrees;
}
...
[[nodiscard]] auto percentile(std::vector<float> values, double p) -> float {
  ...
  auto const index = static_cast<usize>(
      std::clamp(static_cast<usize>(std::lround((values.size() - 1) * p)),
                 static_cast<usize>(0), values.size() - 1));
...
TEST_CASE("png to ktx2 e2e") {
  auto const temp_dir =
      std::filesystem::temp_directory_path() / "texture_tool_tests";
  ...
  std::vector<char const *> args{
      "texture_tool",           "--input",  input_as_string.c_str(), "--output",
      output_as_string.c_str(), "--format", "r8g8b8a8_unorm"};

  EncodeAppConfig app_config{};
  REQUIRE_EQ(
      parse_encode_args(std::span<char const *const>{args.data(), args.size()},
                        app_config),
      CliParseResult::Ok);
...
TEST_CASE("normal texture generates full mip chain") {
  auto const path =
      std::filesystem::path(TEST_ASSETS_DIR) / "images" / "normal.png";
...
TEST_CASE("normal texture mip chain retains positive average z") {
  ...
  for (auto const &mip : chain) {
    ...
    for (usize i = 0; i < mip.pixels.size(); i += 4) {
      auto const n = decode_normal(mip.pixels[i + 0], mip.pixels[i + 1],
                                   mip.pixels[i + 2]);
...
TEST_CASE("normal texture uastc round trips within angular tolerance") {
  ...
  for (usize i = 0; i < original.pixels.size(); i += 4) {
    auto const a = decode_normal(original.pixels[i + 0], original.pixels[i + 1],
                                 original.pixels[i + 2]);

    auto const b =
        decode_normal(decoded.image.pixels[i + 0], decoded.image.pixels[i + 1],
